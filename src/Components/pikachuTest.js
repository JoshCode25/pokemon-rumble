  ...
  getInfo: async function () {
    try {
      ...
      data.moves.map(async function (move) {
        ...
        const moveInfo = {
          //consolidates move info to add to array
          name: moveData.name,
        ...
        if (levelLearned !== 0) {
          //only adds moves the pokemon will learn naturally by leveling up
          pikachu.learnedMovesList.push(moveInfo);
        }
        if (pikachu.level >= levelLearned && levelLearned !== 0) {
      ...
      data.stats.map(async function (stat) {
        ...
        const currentStat = {
          name: statName,
          value: parseInt(baseStatValue, 10) + pikachu.level,
        };
